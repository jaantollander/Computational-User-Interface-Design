...
def solve(elements, positions, frequency, distance, rows, columns,
          colocated=None):
...
# solve the problem
layout, objective = solve(elements, positions, frequency, distance, rows,
                          columns, colocated)

# Print the solution
# TODO: mirror the answer along the axis
print("Objective value (expected selection time):", objective)
print(frequency)
print(np.array(layout).reshape((columns, columns)))
